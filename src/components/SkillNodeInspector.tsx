/**
 * Painel de edição do nó selecionado no canvas.
 *
 * Renderiza os campos específicos por `kind` (gatilho mostra LBs, mensagem
 * mostra textarea + tom, etc). Persistência é responsabilidade do parent
 * via callback `onChange` — este componente é puramente apresentação.
 *
 * Em mobile (≤768px) abre como Sheet inferior; em desktop é um painel lateral.
 */
...
interface SkillNodeInspectorProps {
  ...
  /** Comportamentos disponíveis para o seletor do gatilho */
  behaviorOptions: Array<{ code: string; label: string }>;
  /** Etapas disponíveis para o seletor do gatilho */
      ...
      >
        ...
        <div className="space-y-4 mt-4 pb-8">
          ...
          {node.kind === 'send_message' && (
            <SendMessageFields value={local} onChange={update} />
          )}

          {node.kind === 'wait' && (
            <WaitFields value={local} onChange={update} />
          )}

          {node.kind === 'collect' && (
            <CollectFields value={local} onChange={update} />
          )}

          {node.kind === 'set_tone' && (
            <SetToneFields value={local} onChange={update} />
          )}

          {node.kind === 'handoff' && (
            <HandoffFields value={local} onChange={update} />
          )}

          {node.kind === 'apply_ladder' && (
            <ApplyLadderFields value={local} onChange={update} />
          )}

          {node.kind === 'call_skill' && (
            <CallSkillFields value={local} onChange={update} />
          )}

          {node.kind === 'condition' && (
            <ConditionFields value={local} onChange={update} />
          )}
...
}: FieldProps & { behaviorOptions: Array<{ code: string; label: string }>; stageOptions: Array<{ code: string; name: string }> }) {
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {behaviorOptions.length === 0 && (
            <p className="text-xs text-muted-foreground">Nenhum comportamento cadastrado ainda.</p>
          )}
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <Select
          value={(value.messageType as string) ?? 'text'}
          onValueChange={(v) => onChange({ messageType: v })}
        >
      ...
      <div>
        ...
        <Select
          value={(value.tone as string) ?? 'neutro'}
      ...
      <div>
        ...
        <Input
          ...
          placeholder="ex.: recovery_plan, celebrate_approval"
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <Select
          value={(value.unit as string) ?? 'minutes'}
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <Select
          value={(value.validation as string) ?? 'text'}
          onValueChange={(v) => onChange({ validation: v })}
        >
          <SelectTrigger className="bg-secondary border-border"><SelectValue /></SelectTrigger>
          <SelectContent>
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <Select
          value={(value.priority as string) ?? 'P2'}
  ...
  return (
    <div>
      ...
      <p className="text-xs text-muted-foreground mt-2">
        A skill chamada será executada inline. Ciclos são bloqueados automaticamente.
      </p>
    </div>
  );
}
